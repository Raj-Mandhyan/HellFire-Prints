...
export default function ProductDetails({ product, initialSummary, initialLikesCount, initialIsLiked }: ProductDetailsProps) {
  ...
  // Allowed standard sizes in order
  const ALLOWED_SIZES: readonly string[] = useMemo(() => ['A3', 'A4', 'A5', 'A6'], []);

  // Formatted display dimensions
  const SIZE_DISPLAY_DIMENSIONS: Record<string, string> = useMemo(() => ({
    A3: '29.7 × 42 cm',
    A4: '21 × 29.7 cm',
    A5: '14.8 × 21 cm',
    A6: '10.5 × 14.8 cm',
  ...
  const defaultSizeId = useMemo(() => {
    ...
    return a4?.id || sizes[0]?.id || '';
  }, [sizes]);

  const [customSelectedSizeId, setSelectedSizeId] = useState<string>('');

  // Resolved active size ID (fallback to defaultSizeId if unselected or invalid)
  const selectedSizeId = useMemo(() => {
    if (customSelectedSizeId && sizes.some((s) => s.id === customSelectedSizeId)) {
      return customSelectedSizeId;
    }
    return defaultSizeId;
  }, [customSelectedSizeId, sizes, defaultSizeId]);
  ...
  const selectedVariant = useMemo(() => {
    const matching = product.variants.filter((v) => v.sizeId === selectedSizeId);
    if (matching.length === 0) return undefined;
    if (matching.length === 1) return matching[0];

    // For legacy products with multiple variants per size, pick the base unframed variant
    return (
      matching.find(
        (v) =>
          !v.frameId ||
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="relative aspect-[3/4] w-full bg-neutral-950 border border-neutral-900 rounded-3xl overflow-hidden shadow-2xl shadow-black/90">
            {images.map((img, idx) => (
              <div
                key={img.id || idx}
                className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${activeImageIndex === idx ? 'opacity-100 z-10' : 'opacity-0 z-0'
                  }`}
              >
                <Image
                  src={img.url}
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20 bg-black/40 px-3 py-1.5 rounded-full backdrop-blur-md">
                {images.map((_, idx) => (
                  <button
                    ...
                    className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${activeImageIndex === idx ? 'bg-[#C1121F] scale-125 w-3.5' : 'bg-white/40 hover:bg-white/80'
                      }`}
        ...
        <div className="lg:col-span-5 space-y-8 bg-[#0F0F0F] p-6 sm:p-8 rounded-3xl border border-neutral-900/60 shadow-xl relative overflow-hidden">
          ...
          <div className="space-y-5 pt-6 border-t border-neutral-900">
            ...
            <div className="flex justify-between items-center text-[10px] font-extrabold uppercase tracking-widest bg-neutral-950 p-3.5 border border-neutral-900 rounded-xl">
              ...
              <span className={`${!selectedVariant || selectedVariant.stock <= 0
                  ? 'text-red-500'
            ...
            {feedbackMsg && (
              <div className={`p-4 rounded-2xl border text-xs font-bold uppercase tracking-wider ${feedbackMsg.type === 'success'
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                className={`flex-1 inline-flex items-center justify-center gap-2.5 py-4 rounded-2xl font-black uppercase text-xs tracking-widest transition-all duration-300 disabled:cursor-not-allowed cursor-pointer shadow-lg active:scale-97 ${isJustAdded
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center bg-[#0F0F0F] p-6 sm:p-8 rounded-3xl border border-neutral-900">
            ...
            <div className="md:col-span-3 flex flex-col items-center justify-center text-center space-y-4 h-full py-4 px-0 md:px-4">
              ...
              <button
                ...
                className={`w-full py-3.5 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all duration-300 cursor-pointer active:scale-97 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${isLiked
            ...
            <div className="flex gap-2">
              {(['recent', 'highest', 'lowest'] as const).map((mode) => (
                <button
                  ...
                  className={`px-3 py-2 rounded-xl border text-[9px] uppercase font-black tracking-widest transition-all duration-300 cursor-pointer ${sortBy === mode
